...
static Keyboard::Keyboard GlobalKeyboard;
static Keyboard::KeyboardKey GlobalKeys[128];
...
uint8_t TransformCharcode(Keyboard::Keyboard *Kbd, uint8_t Key) {
    if (Key >= 128) return 0;
...
static void Keyboard_HandleInterrupt_Internal(Keyboard::Keyboard *Kbd, Keyboard::KeyboardKey *Keys, uint32_t KeysMax, uint32_t *KeysLen)
{
    while (IsFull())
    {
        ...
        Keys[*KeysLen] = (Keyboard::KeyboardKey) {
                Kbd->CapsLock,
                Kbd->LShift,
                Kbd->RShift,
                Kbd->LCtrl,
...
void Keyboard::CollectEvents(Keyboard *Kbd, KeyboardKey *Keys, uint32_t KeysMax, uint32_t *KeysLen)
{
    MutexLock(&GlobalKeyboardMutex);
    ...
    *Kbd = GlobalKeyboard;
    kmemcpy((uint8_t *) Keys, (uint8_t *) GlobalKeys, sizeof(KeyboardKey) * KeysToCopy);
